...
.auth-card{background:var(--bg-card);border:1px solid var(--border-color);border-radius:22px;padding:40px;width:100%;max-width:420px;position:relative;z-index:1;}
.logo{display:flex;align-items:center;gap:10px;margin-bottom:30px;}
.logo-icon{width:38px;height:38px;background:linear-gradient(135deg,#7c3aed,#4f46e5);border-radius:11px;display:flex;align-items:center;justify-content:center;flex-shrink:0; color: #fff;}
.logo-text{font-family:'Syne',sans-serif;font-size:18px;font-weight:700;color:var(--text-main);}
.auth-title{font-family:'Syne',sans-serif;font-size:24px;font-weight:700;color:var(--text-main);margin-bottom:5px;}
.auth-sub{font-size:14px;color:var(--text-sub);margin-bottom:26px;}
.lbl{display:block;font-size:12px;color:var(--text-sub);margin-bottom:5px;letter-spacing:.03em;}
.inp{width:100%;background:var(--bg-main);border:1px solid var(--border-color);border-radius:10px;padding:11px 14px;color:var(--text-main);font-family:'DM Sans',sans-serif;font-size:14px;outline:none;transition:border-color .2s;margin-bottom:14px;}
.inp:focus{border-color:#7c3aed;}
.inp::placeholder{color:var(--text-sub); opacity: 0.7;}
...
.auth-switch{text-align:center;margin-top:18px;font-size:13px;color:var(--text-sub);}
...
.sidebar{width:222px;background:var(--bg-card);border-right:1px solid var(--border-color);display:flex;flex-direction:column;padding:0;flex-shrink:0;}
.sb-logo{padding:20px 18px 18px;border-bottom:1px solid var(--border-color);}
.sb-nav{padding:10px 10px 0;flex:1;}
.nav-item{display:flex;align-items:center;gap:10px;padding:10px 12px;cursor:pointer;border-radius:9px;font-size:13.5px;color:var(--text-sub);transition:all .15s;margin-bottom:3px;}
.nav-item:hover{background:var(--hover-bg);color:var(--text-main);}
.nav-item.active{background:rgba(124,58,237,.16);color:#8b5cf6;}
.nav-dot{width:5px;height:5px;border-radius:50%;background:#7c3aed;margin-left:auto;flex-shrink:0;}
.sb-bottom{padding:14px 10px;border-top:1px solid var(--border-color);}
...
.user-name{font-size:13px;font-weight:500;color:var(--text-main);line-height:1.2;}
.user-plan{font-size:11px;color:#7c3aed;}
.logout-row{display:flex;align-items:center;gap:9px;padding:8px 10px;cursor:pointer;color:var(--text-sub);font-size:13px;border-radius:9px;margin-top:2px;transition:all .15s;}
...
.page-title{font-family:'Syne',sans-serif;font-size:22px;font-weight:700;color:var(--text-main);margin-bottom:4px;}
.page-sub{font-size:14px;color:var(--text-sub);}

/* STATS */
.stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px;}
.stat-card{background:var(--bg-card);border:1px solid var(--border-color);border-radius:14px;padding:18px;}
.stat-lbl{font-size:11px;color:var(--text-sub);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px;}
.stat-val{font-family:'Syne',sans-serif;font-size:26px;font-weight:700;color:var(--text-main);margin-bottom:3px;}
...
/* CARD */
.card{background:var(--bg-card);border:1px solid var(--border-color);border-radius:14px;overflow:hidden;margin-bottom:16px;}
.card-hdr{padding:14px 18px;border-bottom:1px solid var(--border-color);display:flex;align-items:center;justify-content:space-between;}
.card-title{font-size:13.5px;font-weight:500;color:var(--text-main);}
.btn-ghost{background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.2);border-radius:7px;padding:6px 12px;color:#8b5cf6;font-size:12px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:background .15s;}
.btn-ghost:hover{background:rgba(124,58,237,.2);}

/* TABLE */
.tbl{width:100%;border-collapse:collapse;}
.tbl th{padding:11px 16px;text-align:left;font-size:11px;color:var(--text-sub);text-transform:uppercase;letter-spacing:.06em;border-bottom:1px solid var(--border-color);}
.tbl td{padding:12px 16px;font-size:13px;color:var(--text-main);border-bottom:1px solid var(--border-color);}
...
.bg{background:rgba(16,185,129,.13);color:#10b981;}
.bb{background:rgba(99,102,241,.13);color:#6366f1;}
.ba{background:rgba(245,158,11,.13);color:#f59e0b;}
.bp{background:rgba(236,72,153,.13);color:#ec4899;}
.bc{background:rgba(14,165,233,.13);color:#0ea5e9;}

/* PROGRESS */
.prog-wrap{padding:14px 18px;}
.prog-bar{background:var(--hover-bg);border-radius:99px;height:6px;overflow:hidden;margin-top:6px;}
...
.dz-title{font-size:14.5px;font-weight:500;color:var(--text-main);margin-bottom:5px;}
.dz-sub{font-size:13px;color:var(--text-sub);}
.preview-img{width:100%;border-radius:11px;object-fit:contain;max-height:190px;margin:14px 0;background:var(--bg-main);}
...
.result-box{background:var(--bg-card);border:1px solid var(--border-color);border-radius:14px;display:flex;flex-direction:column;min-height:300px;}
.result-hdr{padding:13px 16px;border-bottom:1px solid var(--border-color);display:flex;align-items:center;justify-content:space-between;}
.result-body{padding:16px;flex:1;font-size:13px;line-height:1.75;color:var(--text-main);white-space:pre-wrap;overflow-y:auto;font-family:'DM Sans',sans-serif;max-height:380px;}
.copy-btn{background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.2);border-radius:7px;padding:5px 11px;color:#8b5cf6;font-size:12px;cursor:pointer;transition:all .15s;font-family:'DM Sans',sans-serif;display:flex;align-items:center;gap:5px;}
...
.plan-card{background:var(--bg-card);border:1.5px solid var(--border-color);border-radius:16px;padding:24px;cursor:pointer;transition:all .2s;}
...
.plan-name{font-family:'Syne',sans-serif;font-size:18px;font-weight:700;color:var(--text-main);margin-bottom:3px;}
.plan-price{font-family:'Syne',sans-serif;font-size:30px;font-weight:700;color:var(--text-main);margin:11px 0;}
.plan-price span{font-size:14px;color:var(--text-sub);font-weight:400;font-family:'DM Sans',sans-serif;}
.plan-divider{height:1px;background:var(--border-color);margin:14px 0;}
.plan-feat{display:flex;align-items:flex-start;gap:8px;font-size:13px;color:var(--text-sub);margin-bottom:8px;}
.feat-check{color:#10b981;flex-shrink:0;margin-top:1px;}
.subscribe-wrap{margin-top:22px;text-align:center;}
.subscribe-note{font-size:12px;color:var(--text-sub);margin-top:8px;}

/* SETTINGS */
.settings-sec{background:var(--bg-card);border:1px solid var(--border-color);border-radius:14px;padding:22px;margin-bottom:14px;}
.sec-title{font-size:15px;font-weight:500;color:var(--text-main);margin-bottom:3px;}
.sec-sub{font-size:13px;color:var(--text-sub);margin-bottom:18px;}
.form-row2{display:grid;grid-template-columns:1fr 1fr;gap:13px;margin-bottom:13px;}
.inp-sm{width:100%;background:var(--bg-main);border:1px solid var(--border-color);border-radius:8px;padding:9px 12px;color:var(--text-main);font-family:'DM Sans',sans-serif;font-size:13px;outline:none;transition:border-color .2s;}
.inp-sm:focus{border-color:#7c3aed;}
.inp-sm::placeholder{color:var(--text-sub); opacity: 0.7;}
...
.sec-divider{height:1px;background:var(--border-color);margin:14px 0;}
...
/* TOAST */
.toast{position:fixed;bottom:18px;right:18px;background:var(--bg-card);border:1px solid rgba(16,185,129,.3);border-radius:10px;padding:11px 16px;font-size:13px;color:#10b981;z-index:999;box-shadow:0 8px 30px rgba(0,0,0,.15);}
...
export default function App() {
  ...
  const doRegister = async e => {
    ...
    try {
      ...
      setAuthErr("Registration failed. Email might already exist.");
    } finally {
      setAuthLoad(false);
    }
  };
  ...
  const doPay = async () => {
    ...
    try {
      ...
      const options = {
        key: "rzp_test_SZ62Sbymf8U1P2", // Replace with your test key
        amount: order.amount,
        ...
        handler: async function (response) {
          // 3. Handle Success - Update MongoDB
          const label = selPlan.charAt(0).toUpperCase() + selPlan.slice(1);

          try {
            const dbRes = await fetch("http://localhost:8000/api/upgrade-plan", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ email: user.email, plan: label })
            });

            if (!dbRes.ok) throw new Error("Failed to update database");
            const updatedData = await dbRes.json();

            setUser(updatedData.user); // Update React with the fresh MongoDB data
            setPayDone(true);
            showToast(`🎉 Upgraded to ${label} plan!`);
            setTimeout(() => { setPayDone(false); setPage("dashboard"); }, 2200);

          } catch (err) {
            showToast("Payment succeeded, but database update failed.");
          }
        },
        prefill: {
          name: user.name,
        ...
        <div className="scan-layout">
          <div>
            ...
            >
              ...
              <input ref={fileRef} type="file" id="fileup" hidden accept="image/*,application/pdf" style={{ display: "none" }} onChange={e => setFile(e.target.files[0])} />
            </div>
          ...
          <div className="result-box">
            <div className="result-hdr">
              <span style={{ fontSize: "13.5px", fontWeight: 500, color: "var(--text-sub)" }}>Extracted Text</span>
              {extracted && (
                <div style={{ display: "flex", gap: "8px" }}>
                  ...
                  <button
                    className="copy-btn"
                    style={{ color: "#f87171", borderColor: "rgba(239,68,68,.2)", background: "rgba(239,68,68,.08)" }}
                    onClick={() => {
                      setExtracted("");
                      setFile(null);
                      setImgSrc(null);
        ...
        <div className="settings-sec">
          ...
          <div className="form-row2">
            <div>
              ...
              <input id="fname" className="inp-sm" defaultValue={user?.name?.split(" ")[0]} />
            </div>
            <div>
              <div className="lbl">Last name</div>
              <input id="lname" className="inp-sm" defaultValue={user?.name?.split(" ")[1] || ""} />
            </div>
          </div>
